...
export default class ChatController extends ComponentController {
	...
	async searchUsers($form: HTMLFormElement) {
		if (this.isSearching) {
			return;
		}
	...
	deleteChat($form: HTMLFormElement) {
		const SUCCESS_MESSAGE = 'Ура! Чат удален.';
		const formFields = collectFormData($form);
		console.log(formFields);
		new ChatAPI()
	...
	async addUser($form: HTMLFormElement) {
	...
	deleteUser($form: HTMLFormElement) {
		const ERROR_MESSAGE = 'Кажется, такого пользователя нет( Проверьте логин';
		...
		const user = users.filter((user: UserProp) => {
